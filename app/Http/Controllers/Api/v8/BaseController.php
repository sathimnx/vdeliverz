    ...
    public function shops(Request $request){
        try {
            ...
            foreach ($products as $key => $item) {
                ...
                //23-01-2023 for removing prooducts that are not available
                $product_timings = DB::table('product_opencloseTime')->where('product_id', $item['id'])->get();
    ...
    public function vendor_coupondtl(Request $request){
     ...
     $validator = Validator::make($request->all(), [
           ...
           // dd($shop_details);
            ...
            $categories = $products->unique('sub_category_id'); $NextAvailableTime ='';
            foreach ($products as $key => $product) {
                ...
                if(isset($product_timings) && isset($product_timings[0]))
                {
                    foreach ($product_timings as $key => $product_timing) {
                        ...
                        {
                             if($NextAvailableTime == '') { $NextAvailableTime = $start; $isavailable = false; }
                        }
                    }
                }
                      $item = [
                                ...
                                'next_available_time' => $isavailable == false ? ($NextAvailableTime == '' ? '' : 'Next available time '. 
    ...
    public function editchecks(Request $request)
    {
        try {
         $quick = [];
            ...
            $time = now()->format('H:i:s');
            $cat_id = $request->id;
            $collection = \App\Shop::whereHas('products', function($q) use($cat_id){
