...
class CartController extends Controller
{
    ...
    public function getCartData(Request $request){
        try {
            ...
            'delivery_charge' => 0, 'tax' => 0,  'sub_total' => number_format($tc, 2), 'gst_charge' => number_format($gst,2), 'total' => number_format($tc, 2),
            'coupon_amount' => number_format($ca, 2), 'total_afterdiscount'=> number_format($tc + $gst - $ca, 2)]);
        } catch (\Throwable $th) {
            // return apiCatchResponse();
            dd($th);
        }
      ...
      $coupon_code = $request->coupon_code;
        ...
        // dd($coupon->shop_id,$coupon->sub_category_id,$coupon->product_id,$coupon->category_id);
            if(($coupon->sub_category_id > 0 || $coupon->product_id > 0) || $coupon->category_id > 0)
            {
                ...
                {
                    $Shopcoupon = \App\VendorCoupon::where('coupon_code',$request->coupon_code)->where('shop_id',$request->shop_id)->whereDate('expired_on', '>=', now())->first();
                }
                ...
                if(!$Shopcoupon){
                    return response(['status' => false, 'message' => 'Coupon not applicable for this shop!']);
                     ...
                     $cartData = \App\Cart::where('user_id', auth('api')->user()->id)->where('checkout', 0)->where('shop_id',$request->shop_id)
                             ->select('id')->latest()->first();
                 if($cartData != null)
                 {
                     $cartProducts = \App\CartProduct::where('cart_id',$cartData->id)->select(DB::raw('stock_id'))->first();
                     $stock = \App\Stock::where('id', $cartProducts->stock_id)->first();
                        if($Shopcoupon->sub_category_id > 0)
                        {
                            $Product = $Shopcoupon->product_id > 0 ? \App\Product::where('id', $stock->product_id)->whereIn('sub_category_id',array($Shopcoupon->sub_category_id))->
                            where('shop_id',$request->shop_id)
                             ->where('category_id', $Shopcoupon->category_id)->get() :
                            
                            \App\Product::where('category_id', $Shopcoupon->category_id)->whereIn('sub_category_id',array($Shopcoupon->sub_category_id))->where('shop_id',$request->shop_id)->get();
                        ...
                        else if($Shopcoupon->sub_category_id == 0)
                        {
                             $Product = $Shopcoupon->product_id > 0 ? \App\Product::where('id', $stock->product_id)->where('shop_id',$request->shop_id)
                             ->where('category_id', $Shopcoupon->category_id)->get() :
                            
                             \App\Product::where('category_id', $Shopcoupon->category_id)->where('shop_id',$request->shop_id)->get();
                 ...
                 {
                     return response(['status' => false, 'message' => 'This Coupon is not applicable for this shop!']);
                 }
            ...
            if($CouponCount > 0)
            {
                $CartCouponUsedCount = \App\Cart::where('user_id', auth('api')->user()->id)->where('coupon_id',$Shopcoupon->id)->count();
                $discount = 0; 
                if($CartCouponUsedCount <= $Shopcoupon->Discount_use_amt){
                    $cart->vendorcoupon_id = $Shopcoupon->id;
                    if($cart->total_amount >= $Shopcoupon->min_order_amt){
                        $CouponDiscount = round(( str_replace(',', '', $cart->total_amount) / 100 ) * $Shopcoupon->coupon_percentage, 2);
                        $discount = $CouponDiscount > $Shopcoupon->max_order_amount ? $Shopcoupon->max_order_amount : $CouponDiscount;
        ...
        $cart->vendorcoupon_details = json_encode($Shopcoupon);
        $cart->save();
        DB::commit();
