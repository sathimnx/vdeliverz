...
class AccountController extends Controller
{
    ...
    public function myOrders(){
        try {
            ...
            $orders = []; $Shop_opencloseDetails =[];
            foreach ($total_orders as $key => $value) {
               $data = cartProducts($value->cart);
               $shop = \App\Shop::where('id', $value->shop_id)->where('active', 1)->first();
                if($shop != null) { $Shop_opencloseDetails = get_shopopenclosedtl($shop); }
               ...
               $order = [
                  ...
                  'time_left_min' => $value->time_left >= 1 ? 0 : $value->cancel_time['min'],
                   ...
                   "shop_isopened" => $Shop_opencloseDetails == [] || $Shop_opencloseDetails['shopOpened'] =='' ? false : $Shop_opencloseDetails['shopOpened']
                ];
