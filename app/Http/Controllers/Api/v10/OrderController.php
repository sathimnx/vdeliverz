...
class OrderController extends Controller
{
        ...
        if($coupon_code != null)
        {
         ...
         switch (true) {       
           ...
           // }
            ...
            $order->type = 0;
            $order->prefix = '058454';
            $tc = str_replace(',', '', $cart->grand_total);
            ...
            $price_details = [
                ...
                'gst_price' => number_format($gst_charge,2),
                'coupon_discount' => number_format($data['coupon_discount'], 2),
                'total' => number_format($data['total'] , 2),
                'kms' => number_format($distance['int_dis'], 2),
                ];
    ...
    public function orderConfirm(Request $request){
        try {
            ...
            $order->cart->gst_charge = round($gst_charge);
            ...
            $price_details = [
                ...
                'coupon_discount' => number_format($data['coupon_discount'], 2),
                'gst_percentage' => 5,
                'gst_amount'=> number_format($gst_charge, 2),
                'total' => number_format($data['total'], 2),
                ];
    ...
    public function VendorCouponSaveDtl(Request $request)
    {
        try {
            $validator = Validator::make($request->all(),[
                'coupon_code' => ['required'],
            ...
            \App\SubCategory::whereIn('id',array($request->product_category))->select(DB::raw("(GROUP_CONCAT(id SEPARATOR ',')) as `categoryId`"),DB::raw("(GROUP_CONCAT(name SEPARATOR ',')) as `categoryname`"))->get();
            $ProductName = $request->product_dtl =='0' || $request->product_dtl == null ? '0' :
            \App\Product::whereIn('id',array($request->product_dtl))->select(DB::raw("GROUP_CONCAT(id SEPARATOR ',') as productId"),DB::raw("GROUP_CONCAT(name SEPARATOR ',') as productname"))->get();
