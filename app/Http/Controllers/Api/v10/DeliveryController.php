...
class DeliveryController extends Controller
{
    public function deliveryOrders(Request $request){
        try {
            ...
            foreach ($orders as $key => $order) {
                ...
                if(!in_array(auth()->user()->id, $ids)){
                    ...
                    $item = [
                        ...
                        'pick_up_landmark' => $order->shop->landmark == null ? '' : $order->shop->landmark,
                        'pick_up_street' => $order->shop->street,
                        'pick_up_area' => $order->shop->area,
                        'pick_up_city' => $order->shop->city,
                        'drop_off' => $address->address,
                        'drop_off_landmark' => $address->landmark,
            ...
            #Door To Door Delivery(2022-08-10)
            
            $user_id = auth('api')->user()->id;
            
            //dd($user_id);
            $total_orders = DB::table('door_to_doorDelivery')->where('delivery_boy', $user_id)->where('order_status','2')->get();
            ...
            foreach ($total_orders as $key => $value) {
                   ...
                   if(isset($dTd_delivery_orders) ) { 
                     $item = [
                            ...
                            'pickup_name'=>$Pickup_addressDtl->name,
                            ...
                            'drop_name'=> $Dropup_addressDtl->name,
                            'drop_off'=>	$Dropup_addressDtl->address,
                            ...
                            'payment_type' => $dTd_delivery_orders->payment_type,
                            'order_amount' =>round($dTd_delivery_orders->grand_total),
                            'expected_time' => '',
                            'expected_date' => '',
                     ];
                   }
            ...
            return response()->json(['status' => true, 'message' => 'Delivery Orders', 'orders' => $order_items, 'dTd_delivery_orders' => $items, 'count' => $c]);
        } catch (\Throwable $th) {
  ...
  public function acceptOrder(Request $request){
    ...
    public function onGoingOrder(){
        try {
           ...
           //$orders = \App\Order::where('id',25024)->get();
           // dd($orders);
            if(!$orders){
                ...
                //foreach ($delivery as $Delkey => $Delvalue) {
                    ...
                    $item = [
                        ...
                        'pick_up' => $value->shop->address,
                         'pick_up_landmark' => $value->shop->landmark == null ? '' : $orders->shop->landmark,
                        'pick_up_street' => $value->shop->street,
                        'pick_up_area' => $value->shop->area,
                        'pick_up_city' => $value->shop->city,
                        'drop_off' => $address->address,
                        'drop_off_landmark' => $address->landmark,
        ...
        $user = auth('api')->user();
            $user->id = 443;
            $total_orders = DB::table('door_to_doorDelivery')->whereIn('order_status', [4, 6])->where('delivery_boy', $user->id)->get();
            $orders = [];$items = [];$pickup_items=[];
            foreach ($total_orders as $key => $value) {
                   DB::beginTransaction();
      ...
      // dd($OrderDtl);
            return response()->json(['status' => true, 'message' => 'Ongoing Order','d2d_order' => $items, 'order' => $OrderDtl]);
        } catch (\Throwable $th) {
            //return apiCatchResponse();
            dd($th);
        }
    ...
    public function deliveredOrders(){
        try {
            ...
            $user = auth('api')->user();
            $user->id = 3849;
            $total_orders = DB::table('door_to_doorDelivery')->whereIn('order_status', [3])->where('user_id', $user->id)->get();
            $orders = [];$items = [];$pickup_items=[];
            foreach ($total_orders as $key => $value) {
                   DB::beginTransaction();
                   ...
                   if(isset($dTd_delivery_orders) ) { 
                     $item = [
                            'order_id' => $value->order_id,
                            'order_referel' => $value->prefix.$value->order_id,
                            'time' => date('h:i a', strtotime($value->delivered_date)),
                            'date' => date('d-M-Y', strtotime($value->delivered_date)),
                            'date_time' => $value->delivered_date,
                     ];
                   }
                   array_push($items, $item);
            }
           
            return response()->json(['status' => true, 'message' => 'Delivered Orders', 'delivery_boy_type' => Auth::user()->delivery_type == 1 ? false : true, 'points' => $average, 'd2d_orders' =>$items, 'orders' => $delivered]);
        } catch (\Throwable $th) {
            return apiCatchResponse();
            dd($th);
        }
...
//            $order_id = substr($request->search,6);
    ...
    public function canceledOrders(){
        try {
            ...
            foreach ($deliveries as $key => $value) {
                ...
                $user = auth('api')->user();
                $user->id = 3849;
                $total_orders = DB::table('door_to_doorDelivery')->whereIn('order_status', [0])->where('user_id', $user->id)->get();
                $orders = [];$items = [];$pickup_items=[];
                foreach ($total_orders as $key => $value) {
                   DB::beginTransaction();
    ...
    public function accept_d2dOrder(Request $request){
        try {
            $validator = Validator::make($request->all(), [
                'order_id' => 'required'
            ]);
            ...
            $order = DB::table('door_to_doorDelivery')->where('order_id',$request->order_id)->where('order_status', 2)->first();
            
            if(!$order){
                return response()->json(['status' => false, 'message' => 'Order not found']);
            }
            
            DB::table('door_to_doorDelivery')->where('order_id',$request->order_id)->update(array('delivery_boy'=>auth('api')->user()->id,'order_status'=>'4'));
            DB::commit();
            return response()->json(['status' => true, 'message' => 'Order accepted']);
        } catch (\Throwable $th) {
            DB::rollback();
    ...
    public function reject_d2dOrder(Request $request){
        try {
            $validator = Validator::make($request->all(), [
                'order_id' => 'required'
            ]);
            ...
            DB::table('door_to_doorDelivery')->where('order_id',$request->order_id)->update(array('delivery_boy'=>'0','order_status'=>'5'));
            DB::commit();
    ...
    public function ongoing_reject_d2dOrder(Request $request){
        try {
            $validator = Validator::make($request->all(), [
                'order_id' => 'required'
            ]);
            ...
            $order = DB::table('door_to_doorDelivery')->where('order_id',$request->order_id)->where('order_status', 4)->first();
            
            if(!$order){
                return response()->json(['status' => false, 'message' => 'Order not found']);
            }
            
            DB::table('door_to_doorDelivery')->where('order_id',$request->order_id)->update(array('delivery_boy'=>auth('api')->user()->id,'order_status'=>'2'));
            DB::commit();
    ...
    public function dtd_orderDelivered(Request $request){
        try {
            $validator = Validator::make($request->all(), [
                'order_id' => 'required',
                'payment_status' => 'required',
            ...
            $order = DB::table('door_to_doorDelivery')->where('order_id',$request->order_id)->where('order_status', 6)->first();
            
            if(!$order){
                return response()->json(['status' => false, 'message' => 'Order not found']);
            }
            
            DB::table('door_to_doorDelivery')->where('order_id',$request->order_id)->update(array('order_status'=>'3','payment_status'=>$request->payment_status,'cash_note'=>$request->cash_note,
            'delivered_date' => now()));
            DB::commit();
         ...
         //   $CappData = [
           //     'fcm' => $user->fcm,
             //   'title' => config('constants.order_delivered.title'),
             //   'body' => 'Vdeliverz Door To Door Delivery, Hurray ! You received your order, Thanks for shopping with Vdeliverz. Shop more and avail more coupons.',
             //   'icon' => '',
             //   'type' => 1,
            //];
     ...
     public function dtd_orderPicked(Request $request){
        try {
            $validator = Validator::make($request->all(), [
                'order_id' => 'required'
            ]);
            ...
            if($order != null)
            {
                ...
                ->update(array('order_pickedDate'=>Carbon::now(),'order_status' => 6));
