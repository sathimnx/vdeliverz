...
class BaseController extends Controller
{
    ...
    public function dashboard(Request $request){
        try {
            ...
            $wish = auth('api')->user() ? auth('api')->user()->wishlist_count : 0;
            $banners = appBanners();
            $contact = \App\Contact::first(['mobile', 'wp as whatsapp','dTd','email']);
            $shop_banners = \App\ShopBanner::where('active', 1)->inRandomOrder()->limit(8)->get(['id as shop_banner_id', 'image']);
    ...
    public function shops(Request $request){
        try {
            ...
            foreach ($collection as $k => $shop) {
                ...
                {
                     $coupons = \App\VendorCoupon::whereRaw("find_in_set('".$shop->id."',shop_id)")->whereDate('expired_on', '>=', now())->get()->count();
                }
        ...
        try {
           ...
           $coupons=[];
           //dd(\App\VendorCoupon::whereDate('expired_on', '>=', now())->whereRaw("find_in_set('".$request->id."',shop_id)")->get());
            if(\App\VendorCoupon::whereDate('expired_on', '>=', now())->where('active',1)->where('shop_id', 0)->get()->count() > 0)
            {
            ...
            {
                if(\App\VendorCoupon::whereDate('expired_on', '>=', now())->where('active',1)->whereRaw("find_in_set('".$request->id."',shop_id)")
                ->where('category_id',$request->categoryid)
                ->get()->count() > 0)
                {
                   $couponsdtl = \App\VendorCoupon::whereDate('expired_on', '>=', now())->whereRaw("find_in_set('".$request->id."',shop_id)")
     ...
     public function products(Request $request){
       ...
       $validator = Validator::make($request->all(), [
            ...
            {
                 $coupons = \App\VendorCoupon::whereRaw("find_in_set('".$request->shop_id."',shop_id)")->whereDate('expired_on', '>=', now())->get()->count();
            }
...
//            event(new PostPublished($product));
    ...
    {
        try {
              $orders = \App\Order::where('delivered_by',6984)->get();
