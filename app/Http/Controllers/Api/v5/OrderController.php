...
class OrderController extends Controller
{
    ...
    public function orderSummary(Request $request){
        ...
        if($coupon_code != null)
        {
         ...
         switch (true) {       
            ...
            return response()->json(['status' => true, 'message' => 'Order Summary', 'items' => $data['products'], 'order_details' => $order_details, 'price_details' => $price_details, 'available' => false,'coupons' => $coupons]);
        } catch (\Throwable $th) {
            DB::rollback();
    ...
    public function orderConfirm(Request $request){
        try {
            ...
            $gst_charge = $order->shop->gst == 1 ? calculateGSTCharge(str_replace(',', '', $cart->total_amount)) : 0;           
