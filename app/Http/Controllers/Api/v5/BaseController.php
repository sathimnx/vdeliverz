    ...
    public function shops(Request $request){
     ...
     public function products(Request $request){
        try {
            ...
            $items = [];
            $subCategories = [];
            $products = \App\Product::where('shop_id', $request->shop_id)->where('active', 1)->where('rec', 1)->where('category_id', $request->shop_category_id)->with('stocks', 'shop', 'subCategory')->orderBy('updated_at', 'desc')->get();
            if($products->isEmpty()){
            ...
            foreach ($shop->productCategories()->where('shop_sub_category.active', 1)->orderBy('order')->get() as $key => $category) {
             ...
             $coupons = \App\VendorCoupon::where('shop_id', $request->shop_id)->whereDate('expired_on', '>=', now())->where('active',1)->get();
            return response()->json(['status' => true, 'message' => 'Products List', 'shop_details' => $shop_details,'coupons'=>$coupons,  'categories' => $subCategories ?? [], 'shop_products' => $items]);
        } catch (\Throwable $th) {
           return apiCatchResponse();
            dd($th);
        }
