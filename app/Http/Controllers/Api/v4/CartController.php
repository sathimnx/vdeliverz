...
class CartController extends Controller
{
    ...
    public function applyCoupon(Request $request){
        try {
            ...
            $coupon = \App\Coupon::where('coupon_code',$request->coupon_code)->whereDate('expired_on', '>=', now())->first();
                   
            if(!$coupon){
                return response(['status' => false, 'message' => 'Coupon Not Available!']);
            }
            
            //dd(\App\Cart::where('device_id', $request->device_id)->where('coupon_id',$coupon->id)->count());
            $CartCouponUsedCount = \App\Cart::where('user_id', auth('api')->user()->id)->where('coupon_id',$coupon->id)->count();
