...
class CartController extends Controller
{
    ...
    public function getCartData(Request $request){
        try {
            ...
            'delivery_charge' => number_format($dc, 2), 'tax' => 0,  'sub_total' => number_format($tc, 2), 'gst_charge' => number_format($gst,2), 'total' => number_format($tc + $dc - $ca, 2)]);
        } catch (\Throwable $th) {
            // return apiCatchResponse();
            dd($th);
        }
    ...
    public function applyCoupon(Request $request){
      ...
      $coupon_code = $request->coupon_code;
        ...
        if(!$cart)
        {
            return response(['status' => false, 'message' => 'Your order data is not available!']);
        }
       
        $coupon = \App\VendorCoupon::where('coupon_code',$request->coupon_code)->whereDate('expired_on', '>=', now())->where('active',1)->first();
        ...
        $userId= auth('api')->user()->id;
         $CouponCount = 0;
        // dd($coupon->shop_id,$coupon->sub_category_id,$coupon->product_id);
            if($coupon->shop_id > 0 || $coupon->sub_category_id > 0 || $coupon->product_id > 0)
            {
               if($coupon->shop_id == 0)
               {
                   $CouponCount = \App\Cart::where('user_id', $userId)->where('checkout', 0)->select( DB::raw('count(user_id) as count,id'))->latest()->first();
                   $cart_shopId = \App\Cart::where('user_id', $userId)->where('checkout', 0)->latest()->first();
               }
               else
               {
                   $CouponCount = \App\Cart::where('user_id', $userId)->where('checkout', 0)->whereIn('shop_id',array($coupon->shop_id))
                   ->select( DB::raw('count(user_id) as count,id'))->latest()->first();
                    $cart_shopId = \App\Cart::where('user_id', $userId)->where('checkout', 0)->whereIn('shop_id',array($coupon->shop_id))
                   ->latest()->first();
               }
               //dd($CouponCount);
               if($CouponCount->count > 0) {
               
                 $cartProducts = \App\CartProduct::where('cart_id',$CouponCount->id)->select(DB::raw('stock_id'))->first();
                 $stock = \App\Stock::where('id', $cartProducts->stock_id)->first();
                 
                 $Products_subcategoryId = $coupon->sub_category_id == '0' ? \App\Product::whereIn('shop_id', $cart_shopId->shop_id)->get() :
                 \App\Product::where('id', $stock->product_id)->whereRaw('Find_IN_SET(sub_category_id,?)', [$coupon->sub_category_id])->first();
                 
                 //dd($cartProducts,$stock,$Products_subcategoryId);
                 //dd(\App\Product::where('shop_id', $coupon->shop_id)->get());
                    if($coupon->product_id == 0 && $Products_subcategoryId != null)
                    {
                        $CouponCount->count++;
                    }
                         
                    if($coupon->sub_category_id > 0 && $Products_subcategoryId != null)
                    {
                        $CouponCount->count++;
                         $Product = \App\Stock::where('id', $cartProducts->stock_id)->whereRaw('Find_IN_SET(product_id,?)', [$coupon->product_id])->first();
                      
                      //dd($Product);
                         if($coupon->product_id > 0 && $Product != null)
                         {
                             $CouponCount->count++;
                         }
                         else
                    ...
                    else if($coupon->sub_category_id == 0){
                        $CouponCount->count++;
                    }
                    else
                    {
            ...
            if($coupon->shop_id == 0)
            {
                $CartCouponUsedCount = \App\Cart::where('user_id', $userId)->where('coupon_id',$coupon->id)->count();
                $discount = 0;
