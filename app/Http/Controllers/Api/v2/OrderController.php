...
class OrderController extends Controller
{
    public function orderSummary(Request $request){
        try {
            ...
            $address = \App\Address::where('id', $request->address_id)->where('user_id', Auth::user()->id)->first();
            if(!$address){
                return response()->json(['status' => false, 'message' => 'Address not found for this user', 'available' => true]);
            }
            DB::beginTransaction();
            $cart = \App\Cart::where('user_id', auth('api')->user()->id)->where('checkout', 0)->with('order')->first();
            ...
            $distance = scopeIsWithinMaxDistance($cart->shop->latitude, $cart->shop->longitude, $address->latitude, $address->longitude);
            ...
            $order->amount = $tc + $dc + $cart->tax - $ca;
            ...
            $estimate = scopeIsWithinMaxDistance($order->cart->shop->latitude, $order->cart->shop->longitude, $original->latitude, $original->longitude);
            $order_details = [
                'order_id' => $order->id,
            ...
            return response()->json(['status' => true, 'message' => 'Order Summary', 'items' => $data['products'], 'order_details' => $order_details, 'price_details' => $price_details, 'available' => false]);
        } catch (\Throwable $th) {
            DB::rollback();
