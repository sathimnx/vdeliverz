...
class AccountController extends Controller
{
        ...
        try {
            ...
            foreach ($total_orders as $key => $value) {
               ...
               $order = [
                   ...
                   'time_left_min' => $value->time_left >= 5 ? 0 : $value->cancel_time['min'],
                   'time_left_sec' => $value->time_left >= 5 ? 0 : $value->cancel_time['sec'],
                   'can_cancel' => !($value->time_left >= 5),
                ];
    ...
    public function trackOrder(Request $request){
        try {
            ...
            $estimate = scopeIsWithinMaxDistance($order->shop->latitude, $order->shop->longitude, $address->latitude, $address->longitude);

            $order_details = [
                'order_id' => $order->id,
                'order_number' => $order->prefix.$order->id,
                'time_left_min' => $order->time_left >= 5 ? 0 : $order->cancel_time['min'],
                'time_left_sec' => $order->time_left >= 5 ? 0 : $order->cancel_time['sec'],
                'can_cancel' => !($order->time_left >= 5),
