...
class CartController extends Controller
{
    ...
    public function getCartData(Request $request){
        try {
            ...
            if($request->latitude){
                $data = scopeIsWithinMaxDistance($cart->shop->latitude, $cart->shop->longitude, $request->latitude, $request->longitude);
            ...
            return response()->json(['status' => true, 'message' => 'Cart Data', 'currency' => '₹', 'shop_details' => $shop_details,  'products' => $products, 'instructions' => $cart->instructions, 'total_items' => $total_items, 'coupon_discount' => number_format($ca, 2), 'delivery_charge' => number_format($dc, 2), 'tax' => 0,  'sub_total' => number_format($tc, 2), 'total' => number_format($tc + $dc - $ca, 2)]);
        } catch (\Throwable $th) {
            // return apiCatchResponse();
            dd($th);
        }
