    ...
    public function shops(Request $request){
        try {
            ...
            foreach ($collection as $k => $shop) {
                $data = scopeIsWithinMaxDistance($shop->latitude, $shop->longitude, $request->latitude, $request->longitude);
            ...
            return response()->json(['status' => true, 'message' => 'Shops List', 'shop_category_id' => $request->id, 'shops' => collect($shops)->take(2), 'shops1' => collect($shops)->splice(2), 'products' => $quick ?? []]);
        } catch (\Throwable $th) {
     ...
     public function products(Request $request){
        try {
            ...
            foreach ($products->take(8) as $key => $product) {
...
//            event(new PostPublished($product));
    ...
    public function sendfcm(){
        $data = [
            'to' => 'f94_rexnQOWRlhIoVDmweH%3AAPA91bHANlQIGJnWY6BzuTYZ_wLiZODMlE7AEjAuxE1yEv7dHMbprEt-GKUbkl6TSJPyD90L6kmhw0tqYVX-9xt9zBkW8aIwkFfxIPV-QrRK_1A6qAKHJMQGmO8fyoiq6f0NArF2C7tV',
            'data' => [
                "data" => [
                    "title" => "New order",
    ...
    public function orderKms(){
        ...
        foreach ($orders as $key => $order) {
            if(!$order->kms){
                ...
                $distance = scopeIsWithinMaxDistance($order->shop->latitude, $order->shop->longitude, $address->latitude, $address->longitude);
