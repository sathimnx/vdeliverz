...
@section('content')
                ...
                <div class="card-header">
                    <div class="card-text">
                        <div class="row">
                            ...
                            <div class="col-sm-6">
                                @can('create_banners')
                                    <a onclick="showProductCategory('#productCategoryModal', null, null, null, null, null)"
                                        style="cursor: pointer;" class="btn btn-warning float-left text-white">Create Banner</a>
                                @endcan
                                ...
                                <tbody>
                                    @if (isset($banners) && !empty($banners))
                                        @foreach ($banners as $key => $item)
                                            <tr>
                                                <td>{{ $banners->perPage() * ($banners->currentPage() - 1) + $key + 1 }}
                                                </td>
                                                ...
                                                <td>
                                                    <div
                                                        ...
                                                        <input type="checkbox" class="custom-control-input"
                                                            ...
                                                            onchange="change_status('{{ $item->id }}', 'shop_banners', '#customSwitchGlow{{ $item->id }}', 'active');"
                                                ...
                                                @canany(['edit_categories', 'delete_categories'])
                                                    <td>
                                                        <div style="display: inline-flex">
                                                            @can('edit_categories')
                                                                <button type="button"
                                                                    onclick='showProductCategory("#productCategoryModal", "{{ $item->id }}")'
                                                                    class="btn-outline-info" data-icon="warning-alt">
                                                                    <i class="bx bx-edit-alt"></i>
                                                                </button>
                                                            @endcan
                                                            @can('delete_slots')
                                                                <form action="{{ route('shop-banners.destroy', $item->id) }}"
                                                                    class="ml-1"
                                                                    onsubmit="return confirm('Are you sure wanted to delete this Banner?')"
                                                                    method="post">
                                                                    @csrf
                                                                    @method('DELETE')
    ...
    @include('shop_banner._edit_modal')
    @push('scripts')
        <script>
            function showProductCategory(modalId, id = null) {
                if (id != null) {
                    $.ajax({
                        url: "shop-banners/" + id + "/edit",
                        type: 'GET',
                        data: {
                            "_token": '{{ csrf_token() }}'
                        },
                        success: function(response) {
                            ...
                            $(modalId + ' form').attr('action', "{{ url('/shop-banners') }}" + "/" + id);
                    ...
                    $(modalId + ' form').attr('action', "{{ url('/shop-banners') }}");
