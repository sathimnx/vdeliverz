        ...
        <div class="content-body">
            <div class="row">
                <div class="col-md-6">
                    ...
                    @else
                        <input type="hidden" id="filterByType" name="" value="{{ auth()->user()->shop->id }}">
                    @endrole
                </div>
                @push('scripts')
                    <script>
                        function filterOrder(url) {
                            ...
                            window.location.href = url + 'dashboard/' + shop + '/' + year + '/' + month;
                        }
                    </script>
                @endpush
            </div>
                ...
                <div class="col-md-3">
                    <div class="card border text-center my-1">
                        <div class="card-body p-1">
                            <p class="text-muted">Total Orders Delivered <br> <b>{{ $orders_count }}</b></p>
                        </div>
                    </div>
                </div>
                <div class="col-md-3">
                    <div class="card border text-center my-1">
                        <div class="card-body p-1">
                            <p class="text-muted">Total Earnings <br> <b>{{ $earnings }} ₹</b></p>
                        </div>
                    </div>
                </div>
                <div class="col-md-3">
                    <div class="card border text-center my-1">
                        <div class="card-body p-1">
                            <p class="text-muted">Not Delivered <br> <b>{{ $unassigned }}</b></p>
                        </div>
                    </div>
                </div>
                <div class="col-md-3">
                    <div class="card border text-center my-1">
                        <div class="card-body p-1">
                            <p class="text-muted">Vdeliverz Customers <br> <b>{{ $users }}</b></p>
                        </div>
                    </div>
                </div>
                <!-- Column Chart -->
                            ...
                            <div class="d-flex w-100 justify-content-between">
                                ...
                                <div class="form-group w-25">
                                    ...
                                    <select class="form-control"
                                        onchange="change_year('{{ env('APP_URL') }}', this.value)"
                                        id="exampleFormControlSelect1">
                                        @for ($i = $current_year; $i >= 2021; $i--)
                                            <option value="{{ $i }}" {{ $sel == $i ? 'selected' : null }}>
                                                {{ $i }}</option>
                                        @endfor
                        ...
                        <div class="card-header">
                            <div class="d-flex w-100 justify-content-between">
                                ...
                                <div class="form-group w-25">
                                    ...
                                    <?php $current_year = date('Y');
                                    $sel = request()->year ?? $current_year; ?>
                                    <select class="form-control" onchange="changing_both('{{ env('APP_URL') }}')"
                                        id="selectYear">
                                        @for ($i = $current_year; $i >= 2021; $i--)
                                            <option value="{{ $i }}" {{ $sel == $i ? 'selected' : null }}>
                                                {{ $i }}</option>
                                ...
                                <div class="form-group w-25">
                                    ...
                                    $selec = request()->month ?? $current_month; ?>
                                    <select class="form-control" onchange="changing_both('{{ env('APP_URL') }}')"
                                        id="selectMonth">
                                        @for ($j = 1; $j <= 12; $j++)
                                            <option value="{{ $j }}" {{ $selec == $j ? 'selected' : null }}>
                                                {{ date('F', mktime(0, 0, 0, $j, 10)) }}</option>
                                        @endfor
    ...
    @push('scripts')
        ...
        <script>
            function change_year(url, year) {
                var shop = $("#filterByType").val();
                window.location.href = url + 'dashboard/' + shop + '/' + year + '/1#monthlyChart'
            }
            ...
            function changing_both(url) {
                ...
                var shop = $("#filterByType").val();
                var year = $("#selectYear").val();
                window.location.href = url + 'dashboard/' + shop + '/' + year + '/' + month + '#weeklyChart'
            }
        </script>
        <script>
            var e = "#5A8DEE",
