...
@section('content')
                    ...
                    <span>{{$order->cart->centre->user->mobile}}</span>
                                ...
                                <div class="col-md-4 mt-1">
                                    ...
                                    <div class="___class_+?31___">
                                        <span>{{ $address->address }} {{ $address->landmark }} {{ isset($address->google_address) ? $address->google_address : ''  }}  </span>
                                    </div>
                                    {{-- <div class="">
                            ...
                            <table class="table table-borderless mb-0 text-center">
                                <thead>
                                    <tr class="border-0">
                                        ...
                                        <th scope="col" class="font-weight-bolder">Variant</th>
                                ...
                                <tbody>
                                    @forelse ($order->cart->cartProduct as $item)
                                        ...
                                        <tr>
                                            ...
                                            <td>{{ $stock->price }}</td>
                                            <td>{{ $item->count }}</td>
                                            <td>{{ $stock->size ?? $stock->variant . ' ' . $stock->unit }}</td>
                                            <td class="text-primary text-right font-weight-bold">{{ $item->amount }}</td>
                                              <td style="text-align: center; vertical-align: middle;">
                                                 <button type="submit" class="btn-outline-info" onclick="edit_cartProduct({{ $product->id}})" data-icon="warning-alt">
                                                <i class="bx bx-edit"></i>
                                            </button>
                                                <button type="submit" class="btn-outline-danger" onclick="remove_cartProduct({{$item->id}},{{$order->cart->id}})" data-icon="warning-alt">
                                    ...
                                    <div class="col-4 col-sm-4 mt-75">
                                        <div class="invoice-subtotal">
                                            ...
                                            <div class="invoice-calc d-flex justify-content-between mb-1">
                                                ...
                                                <span
                                                    class="invoice-value"></span>
                                                     {{ $order->type == 1 || $order->type == 2 ? 'Online Payment' : 'Cash on Delivery' }} (
                                                        {{ isset($order->deliveredBy->id)
                                                            ? \App\Delivery::where('order_id', $order->id)->where('user_id', $order->deliveredBy->id)->pluck('payment_status')->first()
                                                            : 'NILL' }}
                                                    ) </span>
                                            </div>
                                            <div class="invoice-calc d-flex justify-content-between mb-1">
    ...
    : 'NILL' }}</span>
                                    ...
                                    <div class="col-4 col-sm-4 text-center">
                                        @if ($order->delivered_at == null && $order->canceled_at == null && $order->picked_at == null && $order->rejected_at == null)
                                    ...
                                    <div class="col-4 col-sm-4 d-flex justify-content-end  mt-75">
                                        <div class="invoice-subtotal">
                                            ...
                                            <div class="invoice-calc d-flex justify-content-between mb-1">
                                                ...
                                                <span class="invoice-value">{{ $order->cart->vendorcoupon_amount }} ₹</span>
                                            </div>
                                            <div class="invoice-calc d-flex justify-content-between mb-1">
                                                <span class="invoice-title">Delivery Charge <br>( {{ $order->kms }}
                                                    kms)</span>
        ...
        @push('scripts')
            ...
            <script>
            function edit_cartProduct(product_id)
            {
                $('#newProduct').val(product_id);
                getVariants(product_id,2);
            }
           
                function remove_cartProduct(CartProduct_id,cart_id)
                {
                    var r=confirm("Are you sure you want to delete this Cart Product?");
                    if(r == true)
                    {
                         $.ajax({
                           type:'POST',
                           ...
                           data:{cart_product_id:CartProduct_id,cart_id:cart_id},
                ...
                function getVariants(productValue,type)
                {
                    let productId = type == 1 ? productValue.value : productValue;
                    //let productId= $('#newProduct').val();
                    $.ajax({
                           type:'POST',
                           url:"{{ route('getvariantOfProduct') }}",
                           headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                           data:{product_id:productId},
                           success: function(data) {
                               $('#variant_dtl').html(data.html);
                               $('#actual_price').html('0');
                ...
                {
                    let productId= $('#newProduct').val();
                    let variantId= $('#variant_dtl').val();
                    
                    //alert(variantId);
                    
                    $.ajax({
                           type:'POST',
                           url:"{{ route('getActualPriceOfProduct') }}",
                           headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                           data:{product_id:productId,stock_id:variantId},
                           success: function(data) {
                               //alert(data);
                                 $('#actual_price').html(data);
                ...
                function calculateProductPrice()
                {
                    ...
                    var quantity = $('#quantity_dtl').val();
                    
                    var product_price = actual_price * quantity;
                ...
                {
                    let variantId= $('#variant_dtl').val();
                    let cart_id= $('#hidden_cart_id').val();
                    var quantity = $('#quantity_dtl').val();
                    //alert(cart_id);
                    ...
                    $.ajax({
                           ...
                           url:"{{ route('addToCart') }}",
                           headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                           data:{cart_id:cart_id,stock_id:variantId,quantity:quantity},
                           success: function(data) {
                               //alert(data);
                                // $('#actual_price').html(data);
