...
<div class="row">
    <div class="col-12 col-sm-6">
        ...
        <div class="form-group" id="userEmail">
            <div class="controls">
                <label>E-mail</label>
                <input type="email" class="form-control" placeholder="Email" @if ($route[1] == 'create')
                onkeyup="checkUniqueName('users', 'email', '#userEmail', this.value)" @endif
    ...
    <div class="col-12 col-sm-6">
        ...
        <div class="form-group @role('admin') d-block @else d-none @endrole">
            <div class="controls">
                <label>Role</label>
                <select class="select2 form-control" id="roles" name="roles[]" autocomplete="new-password"
                    data-placeholder="Select Role..." data-validation-required-message="Select user role" required>
                    <option value="">Select Role</option>
                    @if (isset($roles) && !empty($roles))
                        @foreach ($roles as $key => $item)
                            <?php $val = null;
                            if (isset($user)) {
                                $val = $user->roles->pluck('id')->first();
                            } ?>
        ...
        @endpush
         <div class="form-group @if (isset($user) && $user->hasAnyRole('delivery-boy')) d-block @else d-none @endif" id="deliveryBoyCentre">
            ...
            <?php $val = null;
            if (isset($user->delivery_type) && $user->delivery_type != null) {
                $val = $user->delivery_type;
            } ?>
            <select class="select2 form-control" id="centre" name="delivery_type" autocomplete="new-password"
            ...
            <div>
            <label>Driving Preferences for delivery</label>
           <select class="select2 form-control" id="drivepreferences" name="driving_preference[]" multiple="multiple" class="select2-icons form-control select2-hidden-accessible"  autocomplete="new-password"
                data-placeholder="Select Driving Preferences...">
                <?php $val = [];
                    if (isset($user->driving_preference)) {
                        $val = explode(',', $user->driving_preference);
                } ?>
                <option value="Two Wheeler" {{ in_array('Two Wheeler',$val) ? 'selected' : '' }}>Two Wheeler</option>
                <option value="Three Wheeler" {{ in_array('Three Wheeler',$val) ? 'selected' : '' }} >Three Wheeler</option>
                <option value="Four Wheeler" {{ in_array('Four Wheeler',$val) ? 'selected' : '' }} >Four Wheeler</option>
        ...
        <div style="display: flex">
            <fieldset class="controls" style="width: 60%">
                ...
                <div class="input-group">
                    ...
                    <input type="text" value="{{ isset($user) ? str_replace('+91', '', $user->mobile) : '' }}"
                        @if ($route[1] == 'create')
                    onkeyup="checkUniqueName('users', 'mobile', '#userMobile', this.value)" @endif name="mobile"
                    class="form-control" data-validation-required-message="Enter Mobile number"
            ...
            <div class="form-group text-center m-auto" style="width: 40%">
                ...
                <div class="custom-control custom-switch custom-switch-glow custom-control-inline float-right">
                    <input type="checkbox" class="custom-control-input" name="status" <?php if (isset($user->status) && $user->status == 0) {
    echo '';
} else {
    echo 'checked';
} ?>
        ...
        @if (isset($user))
            <div class="form-group">
                ...
                <div class="controls">
                    <input type="password" autocomplete="new-password" name="confirm_password" id="ConfirmPassword"
                        data-validation-match-match="password" class="form-control" placeholder="Repeat Password">
                </div>
            </div>
        @else
            <div class="form-group">
                ...
                <div class="controls">
                    <input type="password" autocomplete="new-password" name="confirm_password" id="ConfirmPassword"
                        ...
                        data-validation-required-message="Repeat password must match" placeholder="Repeat Password">
                </div>
            </div>
        @endif
        ...
        <fieldset class="form-group" id="profile_image">
            ...
            <div class="input-group">
                ...
                <div class="custom-file">
                    <input type="file" class="custom-file-input" name="profile_image" id="storePANImageUpload"
                        aria-describedby="storePANImage">
