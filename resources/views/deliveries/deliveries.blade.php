                                    ...
                                    <div class="d-flex justify-content-between w-100">
                                            ...
                                            <fieldset>
                                                ...
                                                <div class="input-group">
                                                    <input type="number" class="form-control" step=".0000000000000001"
                                                        value="{{ isset($delivery_boy_charge) ? $delivery_boy_charge : old('delivery_boy_charge') }}"
                                                        name="delivery_boy_charge" id="deliveryCharge"
                                        ...
                                        <div class="form-group" style="width: 35%">
                                            <fieldset>
                                                ...
                                                <div class="input-group">
                                                    <input type="number" class="form-control" step=".0000000000000001"
                                                        value="{{ isset($points) ? $points : old('points') }}"
                                                        name="points" id="deliveryPoints" placeholder="Delivery Points"
                                                        aria-describedby="recipePreTimede" required disabled>
                                                    <div class="input-group-append">
                                                        <span class="input-group-text" id="recipePreTimede">Per Order</span>
                                                    </div>
                                                </div>
                                        ...
                                        <div class="form-group m-0" style="width: 20%">
                                            ...
                                            <button type="submit" id="chargeUpdateBtn"
                                                class="btn btn-warning glow mt-2 d-none">Update
                                            </button>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    @if (isset($users) && !empty($users))
                                        @foreach ($users as $k => $item)
                                            <tr>
                                                ...
                                                <td>{{ $item->mobile }}</td>
                                                <td
                                                    class="{{ $item->is_available && $item->active ? 'text-success' : 'text-danger' }}">
                                                    {{ $item->is_available && $item->active ? 'Available' : 'Not Available' }}
                                                </td>

                                                <td>
                                                    @if ($item->onGoing)
                                                        <a href="{{ route('orders.show', $item->onGoing->id) }}"><span
                                                                class="text-primary">{{ $item->onGoing->search }}</span></a>
                                                    @else
                                                        <span class="text-danger">NILL</span>
                                                    @endif
                                                </td>
                                                <td>{{ $item->delivered_count }}</td>
                                                <td>{{ $item->delivery_type == 1 ? 'Monthly' : 'Part time' }}</td>
                                                <td>
                                                    <a href="{{ route('manage-deliveries.show', $item->id) }}"
                                                        class="mr-1">
