            ...
            <div class="card-header">
                <div class="card-text">
                        <div class="row">
                            ...
                            <div class="col-sm-6">
                                @can('create_slots')
                                <a href="{{route('demand.slots.create')}}" style="cursor: pointer;" class="btn btn-warning float-left text-white">Create Slot</a>
                                @endcan
                                ...
                                @push('scripts')
                                    <script>
                                        function filterSlot(url){
                                            ...
                                            window.location.href = url+'demand/slots/'+id+'/filter';
                                        }
                                    </script>
                                @endpush
                                @else
                            ...
                            <tbody>
                                @if (isset($slots) && !empty($slots))
                                    ...
                                    <td>{{($slots->perPage() * ($slots->currentPage() - 1)) + $key + 1}}</td>
                                        <td><a href="{{route('demand.providers.show', $item->provider->id)}}" class="mr-1">{{ucfirst($item->provider->name)}}</a></td>
                                    <td>{{$item->weekdays}}</td>
                                    ...
                                    <td>
                                        ...
                                        <td>
                                            <div style="display: inline-flex">
                                                @can('edit_slots')
                                                <a href="{{route('demand.slots.edit', $item->id)}}">
                                                ...
                                                @endcan
                                                @can('delete_slots')
                                                    <form action="{{route('demand.slots.destroy', $item->id)}}" class="ml-1"  onsubmit="return confirm('Are you sure wanted to delete this Slot?')" method="post">
                                                        @csrf
                                                        @method('DELETE')
